...
module.exports = {
            ...
            example.save(function(err) {
                ...
                if (err) {
                    req.session.messages = {
                        error : 'Could not update example: ' + err
                    };
    ...
    create: function(req, res, next){
        ...
        example.save(function(err) {
            ...
            if (err) {
                req.session.messages = {
                    error : 'Could not create example: ' + err
                };
                res.redirect('/examples');
                return;
            }
    ...
    destroy: function(req, res, next){
        ...
        Example.findById(req.params.id, function(err, example) {
            ...
            if (!example) {
                req.session.messages = {
                    error : 'Unable to locate the example to delete!'
                };
                res.render('404');
                return false;
            };
            ...
            example.remove(function(err) {
                if(err) {
                    req.session.messages = {
                        error : 'There was an error deleting the example!'
                    };
                    res.send('false');
                } else {
                    req.session.messages = {
                        info : 'Example deleted'
                    };
                    res.send('true');
                }
            });
